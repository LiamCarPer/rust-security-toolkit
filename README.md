...
- **Mutation/replay harness (PoC)** - `--poc-template out.json` extracts a mutable, byte-faithful transaction template (legacy and v0-with-ALT round-trip proven); `--poc-run template.json --rpc` rebuilds and simulates it with placeholder signatures (sigVerify disabled server-side, fresh blockhash) and never sends; mutations via `--mutate-account INDEX=PUBKEY`, `--mutate-data IX=HEX`, `--mutate-amount IX=U64`,
  `--mutate-swap IX:A:B`; `--keypair key.json` signs matching signature slots with local keys;
  `--poc-diff A.json B.json` compares the simulated behavior of two templates (exit 1 on any
  change in outcome, CU, or risk flags)
...
- **Bytecode fallback** - when no IDL or source exists (closed-source programs), `--disassemble [PROGRAM_ID]` (or automatically with `--idl-auto`) fetches the on-chain ELF and analyzes it with [sol-azy](https://github.com/FuzzingLabs/sol-azy): syscall inventory, decoded rodata strings, SHA-256 of the binary, upgrade authority, and heuristic dispatch discriminators that name unknown instructions as `dispatch:0x...`; `--output-disassembly DIR` persists the raw disassembly artifacts
